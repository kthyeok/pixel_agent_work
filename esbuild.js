 ...
 * Produces self-contained CJS files with shebangs for provider CLIs to execute.
 */
function buildHooks() {
  const providersDir = path.join(__dirname, 'server', 'src', 'providers', 'hook');
  if (!fs.existsSync(providersDir)) return;
  const entries = fs
    .readdirSync(providersDir, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .flatMap((providerDir) => {
      const hooksDir = path.join(providersDir, providerDir.name, 'hooks');
      if (!fs.existsSync(hooksDir)) return [];
      return fs
        .readdirSync(hooksDir)
        .filter((file) => file.endsWith('-hook.ts'))
