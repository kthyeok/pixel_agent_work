...
Internally, the architecture is fully agent-agnostic and platform-agnostic: a typed `HookProvider` interface defines the integration boundary so adding a new AI tool is a single subdirectory of code. Claude Code and Codex are bundled providers today; Gemini, Cursor, and others are on the roadmap.
...
- Claude Code CLI or Codex CLI installed and configured
...
2. Choose a provider in **Settings** if needed, then click **+ Agent** to spawn a new agent terminal and its character. Right-click for the option to launch in bypass-permissions mode
...
- **Hooks mode** (preferred) — provider hooks POST events (`SessionStart`, `PreToolUse`, `PermissionRequest`, `Stop`, etc.) to a local Fastify server (`POST /api/hooks/:providerId`). Instant, reliable. Server discovery via `~/.pixel-agents/server.json`.
- **Heuristic mode** (fallback) — file-based providers can poll transcript files such as Claude Code JSONL at `~/.claude/projects/<project-hash>/<session-id>.jsonl`. Used when hooks aren't installed.
...
No modifications to the agent CLI are needed — Pixel Agents is purely observational.
...
- **Heuristic-based status detection** — file-based fallback depends on transcript formats that do not always provide clear waiting/finished signals. Hooks mode is preferred; fallback agents may briefly show the wrong status or miss transitions.
- **Linux/macOS tip** — if you launch VS Code without a folder open (e.g. bare `code` command), agents will start in your home directory. This is fully supported; file-based providers may track those sessions using your home directory as the project root.
